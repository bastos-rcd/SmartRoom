...
export default function RoomCard(props: RoomProps) {
  const start = props.startDate ? new Date(props.startDate.replace(" ", "T")) : null;
      ...
      <div className="card-body p-4 d-flex flex-column gap-3">
        ...
        {start && end && (
          <div className="bg-light border border-light-subtle rounded-3 p-3">
            <div className="text-uppercase text-secondary fw-bold small mb-1" style={{ fontSize: "0.7rem", letterSpacing: "0.5px" }}>
              Période réservée
            </div>
        ...
        <div className="d-flex flex-column gap-2">
          <div className="small text-secondary">
            <strong className="text-dark">Capacité :</strong> {props.capacity} places assises
          </div>

          <div className="small text-secondary mt-1">
            <strong className="d-block mb-2 text-dark">Équipements inclus :</strong>
            <div className="d-flex flex-wrap gap-2">
              {props.equipment.map((eq) => (
                <span key={eq} className="badge bg-white text-dark border border-light-subtle rounded-pill px-3 py-2 shadow-sm fs-7 fw-semibold">
                  {eq}
                </span>
              ))}
            </div>
