...
export default function UserSettings() {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        const buildingsMap = (buildingsList || []).reduce((acc, b) => {
          acc[b.id] = b;
          return acc;
        ...
        const userEvents = (allEvents || []).filter((evt) => evt.userId === currentUser.id);
        userEvents.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());
  ...
  return (
    <div className="d-flex min-vh-100 position-relative" style={{ backgroundColor: "#f8fafc" }}>
      ...
      >
        ...
        <div className="p-3 d-flex flex-column gap-2 flex-grow-1">
          <div className="d-flex flex-column gap-1">
            ...
            >
              <span className="sidebar-link-icon" style={{ color: activeTab === "info" ? "#4ade80" : "inherit" }}>
            ...
            <div
              ...
              style={{ textDecoration: "none" }}
            >
              <span className="sidebar-link-icon" style={{ color: activeTab === "reservations" ? "#4ade80" : "inherit" }}>
          ...
          <div className="tab-content animate-fade-in">
            {activeTab === "info" ? (
              <div className="bg-white rounded-4 shadow-sm border border-light-subtle p-4 p-md-5">
                ...
                <form onSubmit={handleSaveInfo} className="d-flex flex-column gap-3">
                  <div className="row g-3">
                    <div className="col-12 col-md-6 text-start">
                      <label className="form-label small fw-bold text-secondary text-uppercase mb-1" htmlFor="firstName">
                        Prénom
                      </label>
                      <input
                        id="firstName"
                    ...
                    <div className="col-12 col-md-6 text-start">
                      <label className="form-label small fw-bold text-secondary text-uppercase mb-1" htmlFor="lastName">
                        Nom
                      </label>
                      <input
                        id="lastName"
                  ...
                  <div className="text-start">
                    <label className="form-label small fw-bold text-secondary text-uppercase mb-1" htmlFor="email">
                      Adresse e-mail
                    </label>
                    <input
                      id="email"
                  ...
                  <div className="text-start">
                    <label className="form-label small fw-bold text-secondary text-uppercase mb-1" htmlFor="password">
                      Nouveau mot de passe <span className="text-muted text-lowercase fw-normal">(laisser vide pour ne pas modifier)</span>
                    </label>
              ...
              <div className="bg-white rounded-4 shadow-sm border border-light-subtle p-4 p-md-5">
                <div className="border-bottom pb-3 mb-4">
                  ...
                  <p className="text-secondary small m-0 mt-1">
                    Visualisez et gérez l'ensemble de vos réservations de salles.
                  </p>
                </div>
                ...
                {reservations.length === 0 ? (
                  ...
                  <div className="d-flex flex-column gap-3">
                    {reservations.map((res) => {
                      ...
                      const roomName = roomObj ? roomObj.name : `Salle #${res.roomId}`;
                      const bldObj = roomObj ? buildings[roomObj.buildingId] : null;
                      const buildingName = bldObj ? bldObj.name : "Bâtiment inconnu";
                      const buildingAddr = bldObj ? bldObj.address : "";
                      const isExpanded = expandedReservation === res.id;

                      const isUpcoming = new Date(res.startDate).getTime() > Date.now();
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            style={{
                              backgroundColor: isExpanded ? "rgba(16, 185, 129, 0.04)" : "transparent",
                              userSelect: "none",
                          ...
                          >
                            <div className="d-flex flex-column align-items-start gap-1">
                              <div className="d-flex align-items-center gap-2 flex-wrap">
                                <span className="fw-bold text-dark fs-5">{roomName}</span>
                                <span
                                  className="badge rounded-pill small px-2.5 py-1"
                                  style={{
                                    backgroundColor: res.type === "confirmed" ? "#d1fae5" : "#fee2e2",
                                    color: res.type === "confirmed" ? "#065f46" : "#991b1b",
                                    fontSize: "0.75rem",
                                    fontWeight: 700,
                                  }}
                                >
                                  {res.type === "confirmed" ? "Confirmée" : "Annulée"}
                                </span>
                                {isUpcoming && (
                                  <span className="badge bg-primary-subtle text-primary rounded-pill small px-2.5 py-1" style={{ fontSize: "0.75rem", fontWeight: 700, backgroundColor: "#dbeafe", color: "#1e40af" }}>
                                    À venir
                                  </span>
                                )}
                              </div>
                              <span className="text-secondary small">
                                Débute le : <span className="fw-semibold text-dark">{formatDate(res.startDate)}</span>
                              </span>
                            </div>

                            <div className="d-flex align-items-center text-secondary">
                              {isExpanded ? (
                                <ExpandLessIcon sx={{ fontSize: "1.8rem", color: "#10b981" }} />
                              ) : (
                            ...
                            >
                              <div className="row g-3 text-start">
                                <div className="col-12 col-md-6">
                                  <div className="mb-2">
                                    <span className="text-muted small fw-bold text-uppercase d-block mb-0.5">Bâtiment & Localisation</span>
                                    <span className="text-dark fw-semibold d-block">{buildingName}</span>
                                    {buildingAddr && <span className="text-secondary small">{buildingAddr}</span>}
                                  </div>
                                  <div className="mb-2">
                                    <span className="text-muted small fw-bold text-uppercase d-block mb-0.5">Étage</span>
                                    <span className="text-dark fw-semibold">{roomObj ? `${roomObj.floor} étage` : "N/A"}</span>
                                  </div>
                                </div>

                                <div className="col-12 col-md-6">
                                  <div className="mb-2">
                                    <span className="text-muted small fw-bold text-uppercase d-block mb-0.5">Période réservée</span>
                                    <span className="text-dark small d-block">
                                      Du : <strong className="text-dark">{formatDate(res.startDate)}</strong>
                                    </span>
                                    <span className="text-dark small d-block">
                                      Au : <strong className="text-dark">{formatDate(res.endDate)}</strong>
                                    </span>
                                  </div>
                                </div>
                              </div>
